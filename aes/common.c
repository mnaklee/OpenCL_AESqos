...
volatile unsigned char *input_file_buffer;
volatile unsigned char temp_buffer[500000/4][4];
volatile unsigned char *output_file_buffer;
volatile unsigned char state[500000/4][4];
int offset_p;
//#pragma omp threadprivate(nr,nk,round_key,key,input_file_buffer,output_file_buffer)
...
void add_round_key(int round)
{
	...
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			state[offset_p+i][j] ^= round_key[round * nb * 4 + i * nb + j];
		}
	}
}
