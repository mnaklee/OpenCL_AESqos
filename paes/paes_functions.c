...
int init_aes(cl_context * context, cl_program * program, cl_command_queue * command_queue, cl_kernel * kernel, cl_mem * cl_buffer, cl_mem * cl_round_key, cl_uchar * buffer, cl_uchar * key, size_t size, opencl_device device, unsigned key_size_bits)
{
...
#ifdef WRITEBUFFER
#else
//    buffer = (cl_uchar*)clEnqueueMapBuffer(*command_queue, *cl_buffer, CL_TRUE, CL_MAP_WRITE, 0, sizeof(cl_uchar) * size, 0, NULL, NULL , &error2);
//    fprintf(stderr,"MapBuffer......\n");
#endif
	cleanup:
	if(!ok)
	{		
...
#else
//    error1 = clEnqueueUnmapMemObject(*command_queue, *cl_buffer, buffer, 0, NULL,NULL);
#endif
...
#else
    buffer = (cl_uchar*)clEnqueueMapBuffer(*command_queue, *cl_buffer, CL_TRUE, CL_MAP_READ, 0, sizeof(cl_uchar) * size, 0, NULL, &event_read, &error);
