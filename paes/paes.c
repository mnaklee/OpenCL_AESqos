	...
	else if(mode == AES_MODE_ENCRYPT)
	{
		...
		while(1)
			...
			//fprintf(stderr, "got timer signal");
				pthread_mutex_lock(&mutex_lock1);
				fgets((char*)buf->buffer, sizeof(cl_uchar)*size, fp);
				pthread_mutex_unlock(&mutex_lock1);		
				pthread_cond_signal(&thread_cond1);
	        }
 ...
 */
void parse_command_line(int argc, char *argv[], char **input_file_name, char **output_file_name, aes_mode * mode, unsigned short *key_size_bits, char **password, opencl_device * device, size_t * size)
{
    ...
    *size = 0;
	do {
		c = getopt(argc, argv, "hi:o:m:k:p:d:g:l:s:");
		switch (c) {
		case 'i':
...
//	sigaddset(&sigset, SIGALRM);
	...
	parse_command_line(argc, argv, &input_file_name, &output_file_name, &mode, &key_size_bits, &password, &device, &size);
	...
	//init aes
	if (init_aes(&context, &program, &command_queue, &kernel, &cl_buffer, &cl_round_key, buffer, password_hash, size, device, key_size_bits) != 1)
	{
		if (buffer)
	                free(buffer);
	        if (input_file_name)
	...
	while (1)
	{
    	...
    	else if(mode == AES_MODE_ENCRYPT)
    	{
			...
			//fprintf(stderr, "got timer signal");
#ifdef WRITEBUFFER
				fgets((char*)buffer, sizeof(cl_uchar)*size, fp);
#else
                buffer = (cl_uchar*)clEnqueueMapBuffer(command_queue, cl_buffer, CL_TRUE, CL_MAP_WRITE, 0, sizeof(cl_uchar) * size, 0, NULL, NULL, NULL);
				fgets((char*)buffer, sizeof(cl_uchar)*size, fp);
                clEnqueueUnmapMemObject(command_queue, cl_buffer, buffer, 0, NULL,NULL);                
#endif
    		//apply aes
	        	if (apply_aes(buffer, &command_queue, &kernel, &cl_buffer, &cl_round_key, size, blocks, key_size_bits, global_size, local_size, mode) != -1)
        		{
